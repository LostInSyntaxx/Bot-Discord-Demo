...
export default new (class VoiceStatsRemoveCommand extends BaseCommand {
  constructor() {
    super({
      name: 'voicestats-remove',
      description: 'Remove voice stats channels from the server',
      category: 'utility',
      cooldown: 10,
      guildOnly: true,
  ...
  async execute(client: ExtendedClient, interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const result = await voiceStatsService.removeVoiceStats(guild.id);
